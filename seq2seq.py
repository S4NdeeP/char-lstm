...
def embedding_attention_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    dtype = scope.dtype
    # Encoder.
    fw_encoder_cell = copy.deepcopy(cell)
    fw_encoder_cell = core_rnn_cell.DropoutWrapper(
          fw_encoder_cell,
          output_keep_prob=dropout_prob,
          seed=1234)


    bw_encoder_cell = cell
    bw_encoder_cell = core_rnn_cell.DropoutWrapper(
          bw_encoder_cell,
          output_keep_prob=dropout_prob,
          seed=1234)

    fw_encoder_cell = core_rnn_cell.EmbeddingWrapper(
        fw_encoder_cell,
        embedding_classes=num_encoder_symbols,
        embedding_size=embedding_size)
    bw_encoder_cell = core_rnn_cell.EmbeddingWrapper(
        bw_encoder_cell,
        embedding_classes=num_encoder_symbols,
        embedding_size=embedding_size)
    
    encoder_outputs, fw_encoder_state, bw_encoder_state = core_rnn.static_bidirectional_rnn(
        fw_encoder_cell,bw_encoder_cell, encoder_inputs, dtype=dtype)

    encoder_state_c = array_ops.concat((fw_encoder_state.c, bw_encoder_state.c), 1)
    encoder_state_h = array_ops.concat((fw_encoder_state.h, bw_encoder_state.h), 1)

    encoder_state = core_rnn_cell.LSTMStateTuple(encoder_state_c, encoder_state_h)
    ...
    decoder_cell = core_rnn_cell.LSTMCell(512)
    decoder_cell = core_rnn_cell.DropoutWrapper(
          decoder_cell,
          output_keep_prob=dropout_prob,
          seed=1234)
